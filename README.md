# Cascade — Blast Radius Agent Powered by IBM Bob
...
[![Hackathon 2026](https://img.shields.io/badge/IBM%20Bob-Hackathon%202026-purple.svg)](https://lablab.ai/event/ibm-bob-hackathon)

**Cascade leverages IBM Bob's unique full-repository context and tool execution capabilities to predict the blast radius of code changes before they merge to production.**

> 🏆 **Built for IBM Bob Hackathon 2026** - Demonstrating how Bob's workspace awareness, file system tools, and multi-file analysis enable system-level impact prediction impossible with snippet-based AI tools.
...
**Cascade is fundamentally built on IBM Bob's unique capabilities:**

### Bob's Tools in Action
- **`read_file`** - Loads up to 5 files simultaneously with full context
- **`search_files`** - Finds all callers across the entire repository
- **`list_files`** - Discovers project structure and dependencies
- **`execute_command`** - Runs git, grep, and analysis commands
- **`list_code_definition_names`** - Maps all exported symbols

### What This Enables
✅ **Full Repository Analysis** - Bob sees the entire codebase, not just snippets
✅ **Cross-Service Impact Detection** - Tracks dependencies across microservices
✅ **Dynamic File Discovery** - Finds affected files automatically
✅ **Configuration Analysis** - Reads .env, docker-compose.yml, K8s manifests
✅ **Complete Call Chain Traversal** - Follows function calls across services

**📖 [Read the full technical deep-dive →](HOW_BOB_POWERS_CASCADE.md)**
...
### **Option 1: Analyze from GitHub URL** 🆕
```bash
# Just paste any GitHub PR URL!
https://github.com/facebook/react/pull/28000

# Cascade automatically fetches and analyzes it
```

### **Option 2: Explore the Demo**
```bash
...
### 🆕 GitHub Integration
- **Paste any GitHub PR URL** - Automatic fetch and analysis
- **Public & Private Repos** - Works with GitHub tokens
- **Zero Manual Work** - No copy-paste needed
- **Real-time Analysis** - Instant blast-radius detection
...
### Getting Started
- **[GitHub Integration Guide](GITHUB_INTEGRATION.md)** - Analyze PRs from GitHub URLs 🆕
- **[How to Run](HOW_TO_RUN.md)** - Setup and installation guide

### IBM Bob Integration
- **[How Bob Powers Cascade](HOW_BOB_POWERS_CASCADE.md)** - Complete technical deep-dive
- **[Bob in Action](bob-reports/BOB_IN_ACTION.md)** - Real tool usage examples
- **[Bob Integration Note](frontend/lib/bob-integration-note.md)** - Implementation details

### Analysis Reports (Generated by Bob)
